  ...
  <div class="container">
    ...
    <div class="search-box">
      ...
      <div class="input-row">
        <div class="input-group">
          ...
          <input v-model="searchForm.name" placeholder="查詢豆商名稱" />
        </div>
      </div>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-content">
        <h3>{{ modalMode==='create' ? '新增豆商' : '編輯豆商' }}</h3>

        <div class="form-row">
          <label>Name</label>
          <input v-model="modalForm.merchantName" />
        </div>
...
async function fetchData () {
  ...
  if (isSearchMode.value) {
    res = await searchMerchants(searchForm.value.name.trim())
  ...
  res = await getMerchants(page.value, size.value)
  merchantList.value = res.data.content
...
function openModal (mode, item=null) {
  ...
  modalForm.value = item ? { ...item } : { id:null, merchantName:'' }
}
...
async function saveData () {
  modalMode.value === 'create'
    ? await createMerchant(modalForm.value)
    : await updateMerchant(modalForm.value)
  closeModal()
  fetchData()
}
